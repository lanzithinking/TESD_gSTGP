...
%                 self.isub=isub(self.stgp,self.K,self.L,self.store_eig); % use on cluster
        ...
        function invmgCv=solve(self,v,woodbury,varargin)
            ...
            if ~self.stgp.spdapx
                ...
                if self.stgp.opt==2 && woodbury
                    ...
                    Phiv=mtimesx(Phi_x,'t',v); % (L,J,K_)
                    PhiinvSPhiv=reshape(Phi_x*reshape(self.isub.solve(Phiv),self.stgp.L,[]),self.stgp.I,self.stgp.J,[]); % (I,J,K_)
                    projv=v-mtimesx(Phi_x,Phiv); % (I,J,K_)
                    invmgCv=self.stgp.C_t.solve(projv,'t')+PhiinvSPhiv; % (I,J,K_)
        ...
        function [M,MU]=sample_postM(self,y)
            ...
            switch self.stgp.ker_opt
                ...
                case 'kron_sum'
                    MU=self.stgp.C_t.mult(reshape(self.solve(Ybar(:),true),self.stgp.I,self.stgp.J),'t');% (I,J)
                    Phi_x=self.stgp.C_x.eigs(self.L);
                    ...
                    M0=self.stgp.C_t.act(projrv,.5,'t')+PhihalfinvSPhirv; % (I,J)
            end
            M=MU+M0;
