...
function []=dynBrainIM_MCMC_holdout_f(grp_opt,mdl_opt,opthypr,jtupt,sampleM,intM,use_para)
sufx={'','_mtimesx','_gpu'};
...
% hold out the last time record for testing
t=t(1:end-1); y=y(:,1:end-1,:);

% obtain dimensions
I=N;
J=size(t,1); K=size(y,3);
L=min([I,40]);
% if I*J>1e4
%     use_para=true;
% end
...
% initializatioin
sigma2=1./gamrnd(a,1./b); sigma2(1)=sigma2(1).^(mdl_opt==1);
...
stgp=feval([stgp_ver,'.hd'],ker{1},ker{2},kappa,Lambda,mdl_opt); [ker{1:2}]=deal([]);
