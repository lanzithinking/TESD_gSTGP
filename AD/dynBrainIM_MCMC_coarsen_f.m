...
function []=dynBrainIM_MCMC_coarsen_f(grp_opt,mdl_opt,opthypr,jtupt,sampleM,intM,use_para)
sufx={'','_mtimesx','_gpu'};
...
sz_y=size(y); imsz_full=sz_y(1:2);

% coarsen the image for refining
imcrsn=4;
y=y(1:imcrsn(1):end,1:imcrsn(end):end,:,:);
...
for gr=1:L_grp
    roi{gr}=roi{gr}(1:imcrsn:imsz_full(1),1:imcrsn:imsz_full(2));
end
