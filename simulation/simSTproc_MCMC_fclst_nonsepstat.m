...
function []=simSTproc_MCMC_fclst_nonsepstat(num_trl,stationary,mdl_opt,upthypr,sampleM,intM,hold_out,spdapx,seedNO)
% sufx={'','_mtimesx','_gpu'};
...
mgC=mg(stgp,K,sigma2(1),L); mgC.stgp.opt=mdl_opt; mgC.stgp.ker_opt=mgC.stgp.jtkers{mgC.stgp.opt+1};
if mgC.stgp.opt==2
%     mgC.isub=feval([stgp_ver,'.isub'],mgC.stgp,mgC.K,mgC.L,mgC.store_eig);
    mgC.isub=isub(mgC.stgp,mgC.K,mgC.L,mgC.store_eig);
end
stgp=[];
% constant updates
