...
function []=simSTproc_MCMC_fclst(num_trl,stationary,mdl_opt,upthypr,sampleM,intM,hold_out,spdapx,seedNO)
% sufx={'','_mtimesx','_gpu'};
...
ker{1}=GP(x,[],[],s,L,jit_x,true);
% temporal kernels
jit_t=1e-6;
% ker{2}=feval([stgp_ver,'.GP'],t,[],[],s,L,jit_t,true,spdapx);
ker{2}=GP(t,[],[],s,L,jit_t,true,spdapx);
% for hyper-GP
ker{3}=ker{2};
...
% stgp=feval([stgp_ver,'.hd'],ker{1},ker{2},Lambda,kappa,mdl_opt,[],[],spdapx);
stgp=hd(ker{1},ker{2},Lambda,kappa,mdl_opt,[],[],spdapx);
[ker{1:2}]=deal([]);
M=stgp.sample_priM; % (I,J)
% set up marginal stgp kernel
% mgC=feval([stgp_ver,'.mg'],stgp,K,sigma2(1),L);
mgC=mg(stgp,K,sigma2(1),L);
stgp=[];
% constant updates
...
keywd=[models{mdl_opt+1},'_I',num2str(I),'_J',num2str(J),'_K',num2str(K),'_L',num2str(L),'_d',num2str(d),'_',repmat('non',1,~stationary),'stationary','_seedNO',num2str(seedNO),'_',time_lbl];
