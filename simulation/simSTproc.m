...
function [x,t,y,L_y]=simSTproc(N,K,d,stationary,seedNO,SAVE,PLOT,d_plt)
...
if ~exist('d_plt','var') || isempty(d_plt)
...
if stationary
    C_xt=exp(-kron(1./(abs(t-t')+1),pdist2(x,x))./(2*l_xt)).*kron(1./(abs(t-t')+1),ones(I));
else
    xt=reshape(x,I,1,[]).*t'; xt=reshape(xt,I*J,[]);
    C_xt=exp(-pdist2(xt,xt,'minkowski',1)./(2*l_xt));
end
...
% file name
f_name=['simSTproc_I',num2str(I),'_J',num2str(J),'_K',num2str(K),'_d',num2str(d),'_',repmat('non',1,~stationary),'stationary'];
% save data to file
if SAVE
    folder = './data/';
