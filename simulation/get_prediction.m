...
M_estm=cell(L_mdl,L_trial,L_seed); [M_estm{:}]=deal(zeros(I,J_tr)); M_estd = M_estm;
...
for sd=1:L_seed
    ...
    %                             M_prestd{mdl+1,tr,sd}=M_prestd{mdl+1,tr,sd}+reshape(diag(Sigma),I,J_te);
            ...
            % calculation mean squared prediction errors
...
save([folder,'pred_',[alg_name,repmat('_intM_',1,intM),'_I',num2str(I),'_J',num2str(J),'_L',num2str(L),'_d',num2str(d),'_',repmat('non',1,~stationary),'stationary'],'.mat'],...
     'M_estm','M_estd','M_predm','M_prestd','C_estm','C_estd','C_predm','C_prestd','MSPE');

%% post-processing

varnames={'mean\\$K=100$','TESD (time)\\$K=100$','TESD (space)\\$K=100$','time\\$K=100$','mean\\$K=1000$','TESD (time)\\$K=1000$','TESD (space)\\$K=1000$','time\\$K=1000$'};
mspe{1}=mean(MSPE,4); mspe{1}=array2table(mspe{1}(:,:),'RowNames',models,'VariableNames',varnames);
